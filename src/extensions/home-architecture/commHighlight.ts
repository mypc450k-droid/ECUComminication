...
function resolveEcuId(name: string): string | null {
  ...
  const match = ecus.find((e) => e.id === lower || e.shortName.toLowerCase() === lower);
  return match?.id ?? null;
}

export function getRecentCommEdgeKeys(messages: CommunicationMessage[]): string {
  const pairs: string[] = [];
...
export function isCommEdgePair(source: string, target: string, keys: string): boolean {
  if (!keys) return false;
  const [a, b] = source < target ? [source, target] : [target, source];
  return keys.includes(`${a}|${b}`);
}
